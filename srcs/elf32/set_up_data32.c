...
static inline void get_direct_values(t_data32 *data, void *p) {
	data->start = p;
	data->ehdr = (Elf32_Ehdr*)p;
	data->shdr = (Elf32_Shdr*)(data->start + data->ehdr->e_shoff);
	data->shstr = (char*)(data->start + data->shdr[data->ehdr->e_shstrndx].sh_offset);
}

static inline void	get_shdr_tabs(t_data32 *data) {
	char	*name;
...
char	set_up_data32(t_data32 **data, char *file, void *p) {
	...
	if (!(*data = (t_data32*)malloc(sizeof(t_data32))))
	...
	if (!((*data)->symtab = (Elf32_Sym*)malloc((*data)->shdr_sym->sh_size))) {
		free(*data);
		return error(file, FATAL_STR, 0);
	}
	ft_memcpy((*data)->symtab, symtab, (*data)->shdr_sym->sh_size);
	(*data)->len_symtab = (*data)->shdr_sym->sh_size / sizeof(Elf32_Sym);
	return 0;
}
