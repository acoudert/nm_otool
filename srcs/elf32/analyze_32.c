...
static void	set_up_section_info(t_sym *syms, t_data32 *data, size_t i) {
	int		shndx;
...
static t_sym	*get_symbols(t_data32 *data) {
	t_sym	*syms = NULL;
	...
	for (size_t i=0; i < data->len_symtab; i++) {
		...
		syms[i].sym_type = ELF32_ST_TYPE(data->symtab[i].st_info);
		syms[i].sym_bind = ELF32_ST_BIND(data->symtab[i].st_info);
		syms[i].sym_vis = ELF32_ST_VISIBILITY(data->symtab[i].st_other);
